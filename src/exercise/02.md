...
J'ai appris à utilise rle hook `useEffect` qui se déclenche à la suite d'un
rendu de composant, et aussi suite à une mise à jour d'une valeur définie par
`useState`. Pour contrôler le déclenchement de `useEffect`, il est possible de
lui passer un tableau de dépendances sur lequelles le hook vas se baser pour
s'exécuter de nouveau. Il est aussi possible de lui passer un tableau de
dépendances vide pour indiquer au hook de s'exécuter qu'une seule fois.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=hdessomme%40gmail.com).
